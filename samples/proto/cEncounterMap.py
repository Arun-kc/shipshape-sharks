...
class EncounterMap:
    ...
    def __init__(
        self,
        decorations_dict: dict = {".": " ", "#": u"\U0001f332"},
        max_width: int = 50,
        max_height: int = 50,
    ...
    def __read_tiles_as_dict(self, tfile: str = r"tiles.txt") -> None:
    ...
    def __create_tiled_row(self) -> str:
        ...
        col_rows = [
            tile.strip().splitlines()
            for tile in np.random.choice(
                list(self.TILES.values()), self.NUM_TILES_HORIZ
            )
        ]

        tiled_row_str = ""
        for row_i in range(len(col_rows[0])):
            tiled_row_str += "".join(col[row_i] for col in col_rows)[:self.MAP_WIDTH]
            tiled_row_str += nl
        return tiled_row_str[:-1]  # remove last new line

    def __create_str_map_from_tiled_rows(self) -> None:
    ...
    def __convert_decorated_str_map_to_matrice(self) -> None:
    ...
    def generate_map(self, tiles_file: str = r"tiles.txt") -> None:
        """Generates Map"""
